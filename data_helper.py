...
def get_stories(story_file):
    readLines = story_file.readlines()
    stories = parse_stories(readLines)
...
def vectorize_stories(data, word2idx, doc_max_len, query_max_len):
    ...
    for s, q, a in data:
        x = [word2idx[w] for w in s if word2idx.get(w)]
        xq = [word2idx[w] for w in q if word2idx.get(w)]
...
#            f = open(dataFile, "rb",encoding="utf-8", errors="replace")
            f = open(dataFile, encoding="utf-8")
            stories = stories + get_stories(f)
